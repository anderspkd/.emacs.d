...
(eval-and-compile
  ;;; Notification service, interface and path variables
  ...
  ;;; List holding events that should be fired when a notification is
  ;;; closed.
  (defvar notify/on-close-events nil)
  (defvar notify/on-close-handler-registered nil)

  ;;; List holding events that should be fired when an action is
  ;;; invoked on a notification.
...
(defun notify (msg &rest args)
  (let ((summary (or (plist-get args :summary) "Hello from Emacs"))
    ...
    ;; Setup handlers. Note that both are essentially no-ops if no
    ;; actions are specified.
    (notify/setup-action-handler)
    (notify/setup-on-close-event-handler)

    ;; Set location (has not been tested)
    (when location
      (if (listp location)
    ...
    ;; Show notification and add the on-close-event, if supplied. We
    ;; have to add the on-close-event here, since we need the
    ;; notification-id to see which notification uses which event.
...
;;;; Takes to long to run (several ms!!)
